...
- The env var `FITTING_BYPASS_PERMISSIONS=true` (read via `config/fitting.config.php`) installs a `Gate::before` hook that returns allowed for every `fitting.*` ability, effectively opening the plugin to every authenticated SeAT user. This is a **staging-only escape hatch** — the test host has it on, production must leave it unset/false. When toggling, run `php artisan config:clear` on the host so the new value is picked up.
...
- **Migrations must be idempotent.** MySQL/MariaDB auto-commit every `ALTER TABLE`, so a migration that fails halfway leaves the schema half-applied while Laravel's `migrations` row never gets written. Re-running then dies on "duplicate column / index" before reaching whatever still needs to apply. Every schema-altering step must check the current state first: `if (! Schema::hasColumn($table, $col)) { ... }`, `SHOW INDEX FROM ... WHERE Key_name = ?` before drop/add. This caught 1.5.0 — see the migration `2026_05_25_010000_add_scope_doctrine_id_to_plan_attachments` for the pattern.
- **Before dropping a UNIQUE/INDEX, check whether it covers a FK column.** MySQL rejects `DROP INDEX` if it's the only index supporting an FK constraint (`ERROR 1553: Cannot drop index ... needed in a foreign key constraint`). If the index you're dropping covers an FK column (typically because it's a composite UNIQUE whose first column is the FK), `ADD INDEX <fk_col>_index (<fk_col>)` first to give the FK a fallback, then proceed with the drop. 1.5.0's `dropUnique('ctsf_plan_attach_unique')` hit this because the UNIQUE's first column was `plan_id` (FK → plans.id) with no other index on it.
